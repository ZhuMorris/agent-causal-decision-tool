...
class TestCompareResultsFixed:
    """Regression: compare_results was broken (return compari undefined)"""

    def test_compare_results_success_path(self):
        """Save two results then compare — was broken at 'return compari'"""
        from src.store import _get_db

        # Clear any existing test records by deleting last 2 rows
        db = _get_db()
        cur = db.cursor()
        # Get current max id to clean up later
        cur.execute("SELECT MAX(id) FROM experiments")
        max_id = cur.fetchone()[0] or 0

        # Save two results (IDs are auto-generated by SQLite)
        r1 = run_action("save_result", {
            "result": {
                "mode": "ab_test",
                "decision": "ship",
                "control_conversions": 100,
        ...
        assert "result" in r1, f"save_result 1 failed: {r1}"
        id1 = r1["result"]["saved_id"]

        r2 = run_action("save_result", {
            "result": {
                "mode": "ab_test",
                "decision": "reject",
                "control_conversions": 100,
        ...
        assert "result" in r2, f"save_result 2 failed: {r2}"
        id2 = r2["result"]["saved_id"]

        # Now compare — was broken at 'return compari' (NameError)
        comp = run_action("compare_results", {"experiment_ids": [id1, id2]}, request_id="cmp")
        assert "result" in comp, f"compare_results failed: {comp}"
        assert "experiments" in comp["result"]

        # Cleanup
        cur.execute("DELETE FROM experiments WHERE id IN (?, ?)", (id1, id2))
        db.commit()


class TestEmptyBatchRejected:
    """Regression: empty batch [] was accepted as valid JSON-RPC"""

    def test_rejects_empty_batch(self):
        _, err = _parse_request([])
        assert err is not None
        assert err["error"]["code"] == -32700
        assert "empty" in err["error"]["message"].lower()


class TestNotificationNoResponse:
    """Regression: notifications (no id) were producing responses"""

    def test_notification_no_response_emitted(self):
        """A notification must not produce any stdout output"""
        from src.api import _get_request_id
        req = {"jsonrpc": "2.0", "method": "decide_ab", "params": {}}
        assert _get_request_id(req) is None  # it's a notification

    def test_request_with_id_does_produce_response(self):
        req = {"jsonrpc": "2.0", "method": "decide_ab", "params": {}, "id": 1}
        assert _get_request_id(req) == 1  # has id = notification


class TestNonDictParams:
    """Regression: non-dict params caused internal errors instead of clean INVALID_PARAMS"""

    def test_non_dict_params_returns_error(self):
        # Array params
        resp = run_action("decide_ab", [], request_id="test")
        assert "error" in resp
        # Null params
        resp2 = run_action("decide_ab", None, request_id="test2")
        assert "error" in resp2


class TestDecideActionAvailable:
    """Regression: 'decide' action missing from method_not_found list"""

    def test_decide_is_valid_action(self):
        """The easy-mode dispatcher 'decide' action must be recognized"""
        resp = run_action("decide", {
            "control_conversions": 100,
            "control_total": 5000,
            "variant_conversions": 130,
            "variant_total": 5000,
        }, request_id="decide-test")
        assert "result" in resp, f"decide action failed: {resp}"
        assert resp["result"]["selected_method"] == "ab_test"

    def test_method_not_found_excludes_decide(self):
        """method_not_found should list 'decide' as a valid action"""
        from src.errors import method_not_found
        err = method_not_found("unknown", request_id=None)
        msg = err.to_jsonrpc(None)["error"]["message"]
        # 'decide' must appear in the valid-actions list
