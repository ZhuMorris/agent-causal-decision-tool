...
@click.option("--format", "output_format", type=click.Choice(["json", "text"]), default="json")
@click.option("--save/--skip-save", "auto_save", default=True, is_flag=True, help="Save result to experiment history (default: True, use --skip-save to disable)")
@click.option("--notify/--skip-notify", "auto_notify", default=False, is_flag=True, help="Fire webhook on ship/reject/escalate decision (default: off, use --skip-notify to disable)")
@click.option("--dry-run", "dry_run", is_flag=True, default=False, help="Validate connector connectivity without running decision")
